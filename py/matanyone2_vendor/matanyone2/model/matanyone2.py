...
class MatAnyone2(nn.Module,
    ...
    def encode_image(self, image: torch.Tensor, seq_length=None, last_feats=None) -> (Iterable[torch.Tensor], torch.Tensor): # type: ignore
        # Follow the actual input tensor device. The upstream demo keeps a
        # module-global default device, which is unsafe for ComfyUI CPU/GPU
        # offload and multi-device workflows.
