...
class RoPEAttention(Attention):
    ...
    def forward(
        ...
        spatial_size = math.isqrt(q.shape[-2])
        if spatial_size * spatial_size != q.shape[-2]:
            raise ValueError(f"RoPE expects a square feature map, got {q.shape[-2]} tokens")
        if self.freqs_cis.is_meta or self.freqs_cis.shape[0] != q.shape[-2]:
            self.freqs_cis = self.compute_cis(
                end_x=spatial_size, end_y=spatial_size
            )
