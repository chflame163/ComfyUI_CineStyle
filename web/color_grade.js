...
function addStyles() {
    ...
    style.textContent = `
      ...
      .cs-grade-viewport{position:relative;width:min(980px,100%);height:clamp(320px,44vh,560px);overflow:hidden;border:1px solid #343943;border-radius:6px;background:#08090b;--compare-position:0%;cursor:default}
      ...
      .cs-grade-image{position:absolute;inset:0;display:block;width:100%;height:100%;object-fit:contain;transform-origin:center center;transition:transform .08s linear}
...
async function openPreview(node) {
    ...
    viewport.addEventListener("wheel", (event) => { event.preventDefault(); setZoom(dialog, dialog._gradeZoom + (event.deltaY < 0 ? 0.1 : -0.1)); }, { passive: false }); viewport.addEventListener("pointerdown", (event) => { if (dialog._gradeZoom <= 1 || event.button !== 0 || event.target === divider) return; viewport.setPointerCapture?.(event.pointerId); viewport.classList.add("pan-active"); dialog._gradePanDrag = { pointerId: event.pointerId, startX: event.clientX, startY: event.clientY, origin: { ...dialog._gradePan } }; }); viewport.addEventListener("pointermove", (event) => { const drag = dialog._gradePanDrag; if (!drag || drag.pointerId !== event.pointerId) return; dialog._gradePan = { x: drag.origin.x + event.clientX - drag.startX, y: drag.origin.y + event.clientY - drag.startY }; clampPan(dialog); applyViewportTransform(dialog); }); const stopPan = (event) => { if (!dialog._gradePanDrag || dialog._gradePanDrag.pointerId !== event.pointerId) return; viewport.releasePointerCapture?.(event.pointerId); dialog._gradePanDrag = null; viewport.classList.remove("pan-active"); }; viewport.addEventListener("pointerup", stopPan); viewport.addEventListener("pointercancel", stopPan);
